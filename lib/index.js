...
var concatOutput = function(builddir, options, files) {
	var status = options.status;
	status.createStage(files.length, 'Concatenating output');
	...
	// ok, concatenate all the articles!
	return P.forEachSeq(files, function(f) {
		var deferred = when.defer();
		var readable = fs.createReadStream(f, { encoding: 'utf8' }).
...
var generateOutput = function(metabook, builddir, options) {
	...
	var files = [ path.join(builddir, 'output.txt') ];
	var output = fs.createWriteStream(files[0], {
		encoding: 'utf8'
	});
	// book or article?
	var hasChapters =
	...
	// XXX word wrap title/subtitle/summary
	output.write(textEscape(title.replace(/\s+/g, ' ')) + '\n');
	if (metabook.subtitle) {
		output.write(textEscape(metabook.subtitle.replace(/\s+/g, ' ')) + '\n');
	}
	output.write('\n');

	if (metabook.summary) {
		output.write(textEscape(metabook.summary.replace(/\s+/g, ' ')));
		output.write('\n\n');
	}
...
var convert = function(options) {
	var status = options.status = new StatusReporter(3, function(msg) {
	...
	return when.resolve().then(function() {
		...
		return concatOutput(builddir, options, files);
	}).then(function() {
