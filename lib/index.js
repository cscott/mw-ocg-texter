...
var Formatter = function(outStream, options) {
	...
	this.columns = options.columns || 75;
...
Formatter.prototype._makeWrap = function(indent, tag) {
	...
	if (tag) {
		...
		return linewrap(indent - this.tabWidth, this.columns, opts);
	}
	return linewrap(indent, this.columns, opts);
};
...
Visitor.prototype.collect = function(node, f) {
	...
	var b = [];
	this.format = {
		fakeFormatFromVisitorCollect: true, // For debugging
		newLine: true,
		newPara: true,
		state: { indent: 0, pre: 0, wrap: null },
		write: function(text) {
			this.newLine = this.newPara = false;
			b.push(text);
		},
...
var generateOutput = function(metabook, builddir, options) {
	...
	if (options.output) {
		...
		// You can pass a stream via options.stream as well.
		writeStream = new stream.Writable({ decodeStrings: true });
		writeStream._write = function(chunk, encoding, callback) {
			return (options.stream || process.stdout).write(chunk, callback);
		};
	}
	...
	write.article = function(item) {
		...
		return pdb.get(key, 'nojson').then(function(data) {
			// avoid redundant parsing in standalone mode (which will
